...
它的核心能力是把普通照片变成有层次感的艺术海报——保留主体，把背景元素拉伸成动态拖影，营造大片质感；同时避免整图均匀模糊，让人物、动作、结构和关键材质继续清晰可辨。
...
Skill 会根据源图中真实存在的人物、花卉、玻璃、光源、视角和动态材料，选择一个主机制，并至多搭配一个辅助机制。机制用于组织画面，不会为了套效果凭空添加源图不存在的元素。

| 视觉机制 | 中文名 | 适合的源图条件 |
|---|---|---|
| `crowd-drift` | 人群漂移 | 人群、公共通道、栈道、交通或重复灯光形成慢门式纵深漂移 |
| `floral-diffusion` | 花卉扩散 | 源图已有花朵、枝叶或植物，可转化为柔和的前景扩散层 |
| `glass-memory` | 玻璃记忆 | 源图已有玻璃、窗户、镜面或可信反射，可形成记忆叠影 |
| `backlit-halation` | 逆光晕染 | 逆光、树冠透光、天空开口或现成灯具形成光晕和光泄漏 |
| `low-angle-color-field` | 低视角色场 | 低视角与大面积天空、墙面或色块共同建立海报式空间 |
| `passing-gesture` | 擦肩手势 | 头发、衣物、手、纸张或树叶等真实材料形成方向性掠影 |
